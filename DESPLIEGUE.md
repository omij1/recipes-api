...
Después crearemos una base de datos externa a la instancia a la que conectaremos nuestra aplicación, dejándola así sin estado (uno de los 
...
ha dejado de funcionar, y para tener una única dirección de acceso a nuestra aplicación. En el caso de que una instancia deje de 
funcionar, se creará automáticamente otra para que siempre estén operativas el número de instancias necesarias para el buen fucionamiento 
de la aplicación, siempre dentro del rango que hayamos configurado (en nuestro caso entre 2 y 10 instancias).
...
la clave a nuestro ssh, ya podemos loguearnos en nuestra instancia a través de la consola mediante el DNS Público de la instancia 
```
ssh ubuntu@PUBLIC_DNS
```
Con el DNS Público podríamos acceder también desde cualquier navegador, ya que hemos habilitado el 
...
En nuestro equipo local, generamos el archivo jar de nuestra aplicación y lo subimos a nuestra instancia. Lo instalamos en 
...
RDS en la consola de AWS. Elegimos el tipo de base de datos. En nuestro caso elegimos PostgreSQL, ya que tiene una gran escalabilidad, 
pudiendo ajustarse al número de cpus y a la cantidad de memoria de que disponga el sistema, con lo que el rendimiento es
mejor (soporta mayor carga de peticiones, aunque es un poco más lenta que MySQL). Además podemos usar una funcionalidad de Play 
válida sólo para bases de datos PostgreSQL (validador @DbArray). 
Comenzamos a configurar nuestra base de datos. En caso de sacar la aplicación a producción elegiríamos esa opción, pero para 
...
```
Ahora cada vez que se pare la aplicación la instancia intentará arrancarla de nuevo.    
...
todas esas peticiones las estaría dirigiendo a una instancia que podría no estar operativa. En caso de un tráfico alto bajaríamos
...
el incremento y disminución de instancias. Para el incremento añadimos una alarma para que salte cuando el uso de la 
